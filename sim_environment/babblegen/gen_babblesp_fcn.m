...
% 10/05/2017:
% Update by R. Ali
% Converting this file into a function so that it can be easily called
% within another m-file to retrieve the noise signal.

function [x,DC] = gen_babblesp_fcn(Fs, c, K, M, d, type_nf, L)
%
%
% x = gen_babblesp_fcn(Fs, c, K, M, d, type_nf, L)
%
% Inputs:
...
% L   % Data length (total number of samples)
%
% Outputs:
% x - babble speech
%
...
% d = 0.01;                  % Inter sensor distance (m)
% type_nf = 'spherical';    % Type of noise field:
%                           % 'spherical' or 'cylindrical'
% L = 20*Fs;                % Data length

% Generate 'independent' babble speech input signals
[data,Fs_data] = audioread('Multitalker_track3_16k.wav');
...
babble = zeros(L,M);
for m=1:M
    babble(:,m) = data((m-1)*L+1:m*L);  % different sections of babble noise.
end
