...
speech_ref = speech(:,1);
%  snr
delta_SI_SNR(rev_i,snr_j) = SI_SNRatio(speech(:,1),noise(:,1),1, fs, vad-SI_SNRatio(s_mwfL(:,1),n_mwfL(:,1),1, fs, vad);

% spectral distance SD
delta_SD(rev_i,snr_j) =  distor_cal(x_STFT,y_STFT,nfft,TX_Mask)-distor_cal(x_STFT,S_mvdr_mwfL_stft,nfft,TX_Mask);
% PESQ
delta_pesq(rev_i,snr_j) = pesq_mex(speech_ref, noisy_sig_ref, fs, 'wideband') - pesq_mex(speech_ref, enhance_sig, fs, 'wideband');
% STOI short time objective Intelligibility Measure
delta_stoi(rev_i,snr_j) = stoi(speech_ref,noisy_sig_ref, fs) - stoi(speech_ref, enhance_sig, fs);
