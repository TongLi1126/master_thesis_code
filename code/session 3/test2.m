...
%             rho_s_esti(k,l) = Rxx_esti{k}(1,1)-rho_rev_realx(k,l);
        ...
        %       noise_power = noisePowMat_y(k,l);
             ...
             *y_STFT(k,l,1);
              % another method
        %       mu = 1;
        %       sig_spectral = sqrt(prioiSNR(k,l)/(mu+prioiSNR(k,l)))*()    rho_rev_realx(k,l)
              Rx3(k,l) = sig_spectral*conj(sig_spectral);
             rho_rev =   rho_r(k,l) ;
%                rho_rev =  0;
              rho_ss  =  abs(Rx3(k,l) ); h = h_steer(:,k);Rn =  Rnn_real{k};
...
subplot(2,1,2); imagesc(1:N_frames,f/1000,mag2db(abs(S_mvdr_mwfL_stft(:,:,1))),[-100 0]); colorbar; axis xy; set(gcf,'color','w');set(gca,'Fontsize',14); xlabel('Time (s)'), ylabel('Frequency (Hz)'),title('Enhanced Signal L - MWF');
% figure;
...
in.wsnr =w_snr(x_STFT(:,:,1),n_STFT(:,:,1),bark); % Compute input SNR
out.wsnr =w_snr(S_mvdr_mwfL_stft(:,:,1),N_mvdr_mwfL_stft(:,:,1),bark) ;% Compute output SNR
delta.WSNR_L = out.wsnr-in.wsnr;
% segsnr
